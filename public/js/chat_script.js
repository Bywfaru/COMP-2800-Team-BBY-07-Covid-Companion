...
function loadList() {
    ...
    if (userChatIdDocRef) {
        ...
        userChatIdDocRef.onSnapshot(function(doc) {
...
function loadActiveMessage(chatId) {
    alert(chatId);
    let chatMessagesColRef = db.collection("Chats").doc(chatId).collection("Messages");
    let chatListDiv = document.getElementById(chatId);
    
    chatListDiv.setAttribute("class", "chat_list active_chat");

    chatMessagesColRef.onSnapshot(function(col) {
        let msgHistoryDiv = document.getElementById("msg_history");
            msgHistoryDiv.innerHTML = "";
        console.log(doc);
        col.forEach(function(message) {
            if (message.data().from == thisUser.uid) {
                ...
                // Set the attributes and inner HTML.
                messageP.innerHTML = message.data().message;
                dateSpan.innerHTML = message.data().time;
