$(document).ready(function(){
  ...
  var searchRes2 = searchRes.split(" ");
  
  console.log(searchRes2);
  
  console.log(searchRes2.length);
  ...
  function loadResults(){
    ...
    .then(querySnapshot => {
     ...
     if(searchRes2.some(keyword => text.includes((keyword).toLowerCase()))
        || searchRes2.some(keyword => text2.includes((keyword).toLowerCase()))
        || searchRes2.some(keyword => text3.includes((keyword).toLowerCase()))) {
    ...
    document.getElementById("postings").appendChild(coldiv); 
          ...
          if(round == querySnapshot.size){
            ...
            var text = document.createTextNode("No Posts Found Under '" + searchRes + "' Keyword(s)");
